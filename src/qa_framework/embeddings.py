...
def get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, index_tensor):
    ...
    if transitive:
        bs_ids = th.nonzero(transitive_mask).squeeze(1)
    ...
    if inter_data is not None:
        inter_transf_data = tuple([x(index_tensor) for x in inter_data])
    else:
        inter_transf_data = None
    return (cen_mul, cen_add, off_mul, off_add), inter_transf_data, (trans_inv, trans_not_inv, projection_dims)
...
def embedding_2i(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])

    c_2, c_2_offset = get_box_data(box_data, data[:, 2])
    transf_data_2, r_2_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 3])
    
    box_c_1 = Box(c_1, c_1_offset).transform(*transf_data_1)#.transform(*r_1_inter)
    box_c_2 = Box(c_2, c_2_offset).transform(*transf_data_2)#.transform(*r_2_inter)
...
def embedding_3i(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])
    ...
    transf_data_3, r_3_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 5])
    
    box_c_1 = Box(c_1, c_1_offset).transform(*transf_data_1)#.transform(*r_1_inter)
    box_c_2 = Box(c_2, c_2_offset).transform(*transf_data_2)#.transform(*r_2_inter)
    box_c_3 = Box(c_3, c_3_offset).transform(*transf_data_3)#.transform(*r_3_inter)

    false_tensor = th.zeros(c_1.shape[0]).bool().to(c_1.device)
    transitive_data = false_tensor, false_tensor, empty_tensor.to(c_1.device)
    return Box.intersection(box_c_1, box_c_2, box_c_3), *transitive_data

def embedding_2in(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # (('e', ('r',)), ('e', ('r', 'n'))): '2in',
    # approximated as 1p
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])
...
def embedding_3in(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # (('e', ('r',)), ('e', ('r',)), ('e', ('r', 'n')))
    # approximated as 2i
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])
...
def embedding_ip(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # ((('e', ('r',)), ('e', ('r',))), ('r',)): 'ip',
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])
...
def embedding_pi(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # (('e', ('r', 'r')), ('e', ('r',))): 'pi',
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, None, data[:, 1])
    transf_data_2, r_2_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 2])
    c_2, c_2_offset = get_box_data(box_data, data[:, 3])
    transf_data_3, r_3_inter, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 4])

    box_c_1 = Box(c_1, c_1_offset).transform(*transf_data_1).transform(*transf_data_2)#.transform(*r_2_inter)
    box_c_2 = Box(c_2, c_2_offset).transform(*transf_data_3)#.transform(*r_3_inter)
...
def embedding_inp(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # ((('e', ('r',)), ('e', ('r', 'n'))), ('r',))
    # approximated as 2p. Therefore, MUST NOT use inter transformation data
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, r_1_inter, _ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, inter_data, data[:, 1])
    transf_data_3, _, _ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, None, data[:, 5])
...
def embedding_pin(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # (('e', ('r', 'r')), ('e', ('r', 'n')))
    # approximated as 2p. Therefore, MUST NOT use inter transformation data
    c_1, c_1_offset = get_box_data(box_data, data[:, 0])
    transf_data_1, *_ = get_role_data(role_data, transitive_ids, inverse_ids, transitive, None, data[:, 1])
...
def embedding_pni(data, box_data, role_data, transitive_ids, inverse_ids, transitive, inter_data):
    # (('e', ('r', 'r', 'n')), ('e', ('r',)))
    # approximated as 1p. Therefore, MUST NOT use inter transformation data
