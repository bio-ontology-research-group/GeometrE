...
class Box():
    def __init__(self, lower_corner, delta, pos_delta=True):
        ...
        self.center = (self.lower_corner + self.upper_corner)/2
    ...
    def inclusion(box1, box2, margin, *args):
        ...
        # return th.norm(lower_corner_condition, p=1, dim=1) + th.norm(upper_corner_condition, p=1, dim=1)
        containment_score = th.linalg.norm(lower_corner_condition, axis=1) + th.linalg.norm(upper_corner_condition, axis=1)

        # proximity_score = th.linalg.norm(box1.lower_corner - box2.lower_corner, axis=1)
        # proximity_score = th.norm(box1.lower_corner - box2.lower_corner, p=1, dim=1)
        proximity_score = (box1.center - box2.center).pow(2).sum(dim=1)
        # return containment_score + proximity_score
...
def normal_gci2_loss(data, class_lower, class_delta, rel_embed, rel_mask, transitive_ids, margin, neg=False):
    ...
    norm_c = th.linalg.norm(c, axis=1)
    norm_d = th.linalg.norm(d, axis=1)
