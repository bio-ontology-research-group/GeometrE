...
from gda_evaluator import GDAEvaluator
from gda_dataset import GDADatasetEL
...
@ck.command()
@ck.option("--dataset_name", "-ds", type=ck.Choice(["gda", "gda_simpler"]), default="gda")
@ck.option("--evaluator_name", "-e", default="gda", help="Evaluator to use")
...
@ck.option("--max_bound", "-mb", default=10, help="Maximum bound for the embeddings")
...
def main(dataset_name, evaluator_name, embed_dim, batch_size,
         ...
         transitive, max_bound, device, wandb_description, no_sweep, only_test):
    ...
    model_filepath = f"{model_dir}/{embed_dim}_{batch_size}_{module_margin}_{loss_margin}_{learning_rate}_{transitive}_{max_bound}.pt"
    model = GeometricELModel(evaluator_name, dataset, batch_size,
                             ...
                             filter_deductive, transitive, max_bound, device, wandb_logger)
...
def dataset_resolver(dataset_name):
    if dataset_name.lower() == "gda":
        root_dir = "../../use_cases/gda/data/"
    elif dataset_name.lower() == "gda_simpler":
        root_dir = "../../use_cases/gda_simpler/data/"
    else:
...
class GeometricELModel(EmbeddingELModel):
    def __init__(self, evaluator_name, dataset, batch_size, embed_dim,
                 ...
                 evaluate_deductive, filter_deductive, transitive, max_bound,
                 device, wandb_logger):
        ...
        logger.info(f"Number of transitive properties: {len(self.transitive_ids)}")
        ...
        print(f"Number of individuals: {len(self.dataset.individuals)}")
    ...
    def train(self):
        ...
        all_classes_to_idx = {c: i for i, c in enumerate(self.dataset.classes.as_str)}
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if epoch % self.evaluate_every == 0:
                ...
                self.wandb_logger.log(valid_macro_metrics)
