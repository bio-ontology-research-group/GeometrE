...
@check_output_shape
def gci2_loss(data, class_lower, class_delta, rel_embed, rel_mask, transitive_ids, margin, transitive, neg=False, train=False): #adapted

    if not transitive:
        return normal_object_property_assertion_loss(data, class_lower, rel_embed, rel_mask, transitive_ids, margin, neg = neg)

    return object_property_assertion_loss(data, class_lower, rel_embed, rel_mask, transitive_ids, margin, transitive, neg = neg, train=train)
...
# @check_output_shape
def object_property_assertion_loss(data, individual_embed, rel_embed, rel_mask, transitive_ids, margin, transitive, neg=False, train=False):
    ...
    if neg:
        ...
        transitive_loss = Box.order_loss(box_c_trans, box_d_trans, margin)
    else:
