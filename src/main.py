...
def evaluate(model, tp_answers, fn_answers, transitive_answers, args, dataloader, query_name_dict, mode, step, writer, wb_logger):
    '''
    ...
    metrics = model.test_step(model, tp_answers, fn_answers, transitive_answers, args, dataloader, query_name_dict)
...
def load_data(args, tasks):
    ...
    if args.deductive_negative_sampling:
        transitive_train_answers = pickle.load(open(os.path.join(args.data_path, "train-transitive-answers.pkl"), 'rb'))
    else:
        transitive_train_answers = None
    ...
    if args.filter_deductive_triples:
        test_transitive_answers = pickle.load(open(os.path.join(args.data_path, "test-transitive-answers.pkl"), 'rb'))
    else:
        test_transitive_answers = None
    ...
    return train_queries, train_answers, transitive_train_answers, valid_queries, valid_tr_queries, valid_hard_answers, valid_easy_answers, test_queries, test_tr_queries, test_hard_answers, test_easy_answers, test_transitive_answers, transitive_ids, inverse_ids
...
def main(args):
    ...
    if args.geo in ['pure_box']:
        tmp_str = "hid-{}-g-{}-a-{}-lr-{}-bs-{}-ns-{}-trans-{}-seed-{}".format(args.hidden_dim, args.gamma, args.alpha, args.learning_rate, args.batch_size, args.negative_sample_size, args.transitive, args.seed)
    ...
    if args.do_train:
        ...
        train_path_iterator = SingledirectionalOneShotIterator(DataLoader(
                                    TrainDataset(train_path_queries, nentity, nrelation, args.negative_sample_size, train_answers, transitive_train_answers),
                                    batch_size=args.batch_size,
                                    shuffle=True,
        ...
        if len(train_other_queries) > 0:
            ...
            train_other_iterator = SingledirectionalOneShotIterator(DataLoader(
                                        TrainDataset(train_other_queries, nentity, nrelation, args.negative_sample_size, train_answers, transitive_train_answers),
                                        batch_size=args.batch_size,
                                        shuffle=True,
                                        num_workers=args.cpu_num,
                                        collate_fn=TrainDataset.collate_fn,
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.valid_steps == 0 and step > 0:
                ...
                if args.do_test:
                    ...
                    test_all_metrics = evaluate(model, test_easy_answers, test_hard_answers, test_transitive_answers, args, test_dataloader, query_name_dict, 'Test', step, writer, wandb_logger)
                    if args.do_test_tr:
                        logging.info('Evaluation on Transitive Test Dataset:')
                        test_tr_all_metrics = evaluate(model, test_easy_answers, test_hard_answers, test_transitive_answers, args, test_tr_dataloader, query_name_dict, 'TestTr', step, writer, wandb_logger)
