...
class TrainDataset(Dataset):
    def __init__(self, queries, nentity, nrelation, negative_sample_size, answer, transitive_answer):
        # queries is a list of (query, query_structure) pairs
        self.len = len(queries)
        self.queries = queries
        self.nentity = nentity
        ...
        if transitive_answer is None:
            self.transitive_answer = {v: set() for v, _ in self.answer.items()}
        else:
            self.transitive_answer = transitive_answer 
    ...
    def __getitem__(self, idx):
        ...
        while negative_sample_size < self.negative_sample_size:
            ...
            answer = self.answer[query] | self.transitive_answer[query]
            mask = np.in1d(
                negative_sample, 
                answer,
                assume_unique=True, 
